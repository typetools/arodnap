...
NEGATIVE_REPO_NAME = "apache/zookeeper"
NEGATIVE_REPO_SHA = "83221ec57e71a67877c17f71dcbf2cd3546f81aa"
NEGATIVE_REPO_URL = "https://github.com/apache/zookeeper.git"
NEGATIVE_REPO_DIRNAME = "apache-zookeeper"
EXPECTED_MAVEN_SHAPE_FAILURE_FRAGMENTS = (
    "Gradle repo root must contain one of",
    "Multi-module Gradle repos are not supported in v1.",
)


@dataclass(frozen=True)
class Step5MavenShapeNegativeResult:
    repo_name: str
    pinned_sha: str
    repo_root: Path
...
def build_parser(defaults: ValidationSessionConfig | None = None) -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(prog="python -m arodnap.validation_negative_maven_shape")
    add_validation_session_arguments(parser, defaults=defaults)
    return parser


def run_step5_maven_shape_negative(
    ...
    *,
    ...
    def fail(reason: str, *, exit_code: int = 1) -> Step5MavenShapeNegativeResult:
    ...
    if not clone.success:
        return fail(
            render_command_failure("Step 5 failed while cloning the negative repo.", clone),
    ...
    if not fetch.success:
        return fail(
            render_command_failure("Step 5 failed while fetching the pinned negative SHA.", fetch),
    ...
    if not checkout.success:
        return fail(
            render_command_failure("Step 5 failed while checking out the pinned negative SHA.", checkout),
    ...
    if not status_result.success:
        return fail(
            render_command_failure("Step 5 failed while checking repo status after analyze.", status_result),
    ...
    if not analyze_failed:
        return fail(
            "Step 5 expected `arodnap analyze` to reject the unsupported Maven multi-module shape, "
            "but it exited successfully."
        )

    if not repo_clean:
        return fail(
            "Step 5 detected unexpected repo mutations after the negative analyze run.\n"
    ...
    if failed_checks:
        return fail(
            "Step 5 analyze failed, but not with the expected explicit early Maven multi-module "
            "or non-Gradle-root rejection.\n"
            "Failed checks:\n"
            + "\n".join(failed_checks)
            + "\n"
...
def _evaluate_contract_checks(analyze: ValidationCommandResult, out_analyze: Path) -> dict[str, bool]:
    ...
    return {
        "command_output_mentions_maven_shape_contract": _contains_maven_shape_failure(combined_output),
        ...
        "manifest_error_mentions_maven_shape_contract": (
            manifest_payload is not None
            and _contains_maven_shape_failure(str(manifest_payload.get("error", "")))
        ),
        ...
        "report_error_mentions_maven_shape_contract": (
            report_payload is not None
            and _contains_maven_shape_failure(str(report_payload.get("error", "")))
        ),
...
def _contains_maven_shape_failure(text: str) -> bool:
    return any(fragment in text for fragment in EXPECTED_MAVEN_SHAPE_FAILURE_FRAGMENTS)
