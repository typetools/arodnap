...
from arodnap.validation_negative_repo_root import (
    NEGATIVE_REPO_DIRNAME,
    NEGATIVE_REPO_NAME,
    NEGATIVE_REPO_SHA,
    Step4RepoRootNegativeResult,
    main,
    run_step4_repo_root_negative,
)
...
class ValidationNegativeRepoRootTest(unittest.TestCase):
    def test_run_step4_repo_root_negative_accepts_explicit_early_rejection(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("arodnap.validation_negative_repo_root.setup_validation_session", return_value=session):
                with patch("arodnap.validation_negative_repo_root.run_validation_command", side_effect=fake_run):
                    result = run_step4_repo_root_negative(config, env={"PATH": "/usr/bin"})
    ...
    def test_run_step4_repo_root_negative_fails_when_analyze_succeeds(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            session = self._make_session(temp_root)
            config = ValidationSessionConfig(
    ...
    def test_run_step4_repo_root_negative_fails_when_rejection_is_ambiguous(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            session = self._make_session(temp_root)
            config = ValidationSessionConfig(
        ...
        self.assertFalse(result.success)
        self.assertTrue(result.analyze_failed)
        self.assertIn("expected explicit early repo-root contract rejection", result.failure_reason)
        self.assertFalse(result.contract_checks["command_output_mentions_repo_root_contract"])
        self.assertFalse(result.contract_checks["report_final_analysis_absent"])

    def test_run_step4_repo_root_negative_fails_when_repo_is_dirty(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_root = Path(temp_dir)
            session = self._make_session(temp_root)
            config = ValidationSessionConfig(
            ...
            def fake_run(command, *, label, cwd, env=None):
                ...
                if label == "analyze":
                    self._write_failed_analysis_outputs(
                        out_analyze,
                        error_message="Gradle repo must use src/main/java in v1.",
                    )
                    return ValidationCommandResult(
                        label=label,
                        command=list(command),
                        ...
                        success=False,
                        stdout="",
                        stderr="AnalyzeError: Gradle repo must use src/main/java in v1.\n",
                        invocation_error=None,
                    )
    ...
    def test_main_prints_json_summary(self) -> None:
        ...
        with patch("arodnap.validation_negative_repo_root.run_step4_repo_root_negative", return_value=result):
